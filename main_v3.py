...
def main(config, seed = 1337):
    ...
    df_test['fold'] = 99
    if config.K_FOLD > 1:
        indices = split_K_fold(config.K_FOLD, df_train, df_train[['subject_number']])
        for fold, inds in enumerate(indices, start = 1):
            df_train.loc[inds, 'fold'] = fold
    assert len(df_train['fold'].unique()) == config.K_FOLD
...
def split_K_fold(k, X, group):
    """
    Split data into K folds based on patient IDs
    Output:
        train_indices: training set indices for each split
        val_indices: validation set indices for each split
    """
        
    kfold = GroupKFold(n_splits = k)
    
    val_indices = []
    for _, val_idx in kfold.split(X, None, group):
