...
def main(config, seed = 1337):
    ...
    if (config.K_FOLD > 1): # Assign subjects manually
        # Fold 1
        fold_1_ids = ['201-005', '201-006', '201-014', '201-017', '201-028', '201-033', '202-014', 
                      '202-039', '202-053', '202-055', '202-063', '203-004', '203-015', '203-025',
                      '203-031', '203-036', '203-037', '203-048', '203-072', '203-085', '203-068',
                      '203-071', '203-022']
        assign_1 = df_train['subject_number'].isin(fold_1_ids)
        df_train.loc[assign_1, 'fold'] = 1

        # Fold 2
        fold_2_ids = ['201-007', '201-024', '201-034', '202-007', '202-009', '202-025', '202-029',
                      '202-030', '202-041', '202-051', '202-072', '202-075', '203-014', '203-018',
                      '203-020', '203-028', '203-038', '203-043', '203-047', '203-050', '203-088',
                      '205-003', '201-002', '202-077']
        assign_2 = df_train['subject_number'].isin(fold_2_ids)
        df_train.loc[assign_2, 'fold'] = 2
        
        # Fold 3
        fold_3_ids = ['201-003', '201-011', '201-019', '201-032', '202-013', '202-021', '202-022',
                      '202-037', '203-003', '203-013', '203-021', '203-026', '203-045', '203-046',
                      '203-061', '203-067', '203-082', '203-083', '203-086', '202-074', '202-036',
                      '203-058', '203-077', '203-080']
        assign_3 = df_train['subject_number'].isin(fold_3_ids)
        df_train.loc[assign_3, 'fold'] = 3
        
        # Fold 4
        fold_4_ids = ['201-001', '201-013', '201-025', '201-027', '202-004', '202-017', '202-023',
                      '202-043', '202-064', '202-080', '202-083', '203-010', '203-011', '203-033',
                      '203-034', '203-052', '203-057', '203-065', '203-069', '203-070', '203-056',
                      '203-076', '203-079', '203-055']
        assign_4 = df_train['subject_number'].isin(fold_4_ids)
        df_train.loc[assign_4, 'fold'] = 4
        
        # Fold 5
        fold_5_ids = ['201-009', '201-012', '201-015', '201-021', '201-031', '202-018', '202-028',
                      '202-035', '202-046', '202-076', '203-029', '203-030', '203-039', '203-041',
                      '203-053', '203-060', '203-066', '203-074', '203-089', '205-001', '205-002',
                      '202-079', '203-064']
