...
def main(config, seed = 1337):
    ...
    if (config.K_FOLD > 1): # Assign subjects manually
        # Fold 1
        fold_1_ids = ['201-024', '201-028', '201-030', '201-032', '201-033', '201-035', '202-011', 
                      '202-044', '202-049', '202-062', '202-073', '203-011', '203-036', '203-037', 
                      '203-043', '203-053', '203-057', '203-067', '203-069', '203-074', '203-085', 
                      '203-088', '205-003', '202-074', '203-003', '203-077']
        assign_1 = df_train['subject_number'].isin(fold_1_ids)
        df_train.loc[assign_1, 'fold'] = 1

        # Fold 2
        fold_2_ids = ['201-001', '201-003', '201-006', '201-016', '202-004', '202-013', '202-047', 
                      '202-067', '202-076', '203-006', '203-030', '203-033', '203-041', '203-050', 
                      '203-051', '203-066', '203-083', '201-021', '202-077', '202-079', '203-064', 
                      '203-076', '203-080', '203-022', '203-026', '203-055']
        assign_2 = df_train['subject_number'].isin(fold_2_ids)
        df_train.loc[assign_2, 'fold'] = 2
        
        # Fold 3
        fold_3_ids = ['201-005', '201-013', '201-014', '201-025', '201-034', '202-014', '202-025', 
                      '202-031', '202-037', '202-041', '202-042', '202-043', '202-046', '202-050', 
                      '202-053', '202-072', '203-001', '203-005', '203-010', '203-024', '203-039', 
                      '203-045', '203-048', '203-070', '202-007', '202-036']
        assign_3 = df_train['subject_number'].isin(fold_3_ids)
        df_train.loc[assign_3, 'fold'] = 3
        
        # Fold 4
        fold_4_ids = ['201-015', '201-029', '201-031', '202-008', '202-022', '202-040', '202-052', 
                      '202-055', '202-064', '202-075', '202-078', '202-081', '203-002', '203-004', 
                      '203-015', '203-025', '203-028', '203-029', '203-044', '203-047', '203-084', 
                      '201-002', '203-056', '203-058', '203-068', '203-079']
        assign_4 = df_train['subject_number'].isin(fold_4_ids)
        df_train.loc[assign_4, 'fold'] = 4
        
        # Fold 5
        fold_5_ids = ['201-010', '201-011', '202-009', '202-010', '202-028', '202-029', '202-030', 
                      '202-048', '202-054', '202-070', '203-012', '203-013', '203-014', '203-018', 
                      '203-021', '203-031', '203-034', '203-038', '203-046', '203-060', '203-072', 
                      '203-086', '203-087', '203-091', '203-071']
