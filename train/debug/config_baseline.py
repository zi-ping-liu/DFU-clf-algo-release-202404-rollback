...
class Config():
    ...
    def __init__(self,
        ...
        #########################################################################
        #                            TRAINING DETAIL
        self.EXPORT_PATH = "/home/efs/ziping/workspaces/dfu/clf_algo_release_202404_rollback/results/debug/" + self.MODEL.strip().lower()
...
if __name__ == "__main__":
    ...
    train_csv = "/home/efs/ziping/workspaces/dfu/clf_algo_release_202404_rollback/src/data/WAUSI_unifiedv5_BSV+slidingwindow_reverted_newsplit_20240422_updated.csv"
    df = pd.read_csv(train_csv)
    
    if image_preprocess['USE_GOOD_ORI_ONLY']: # Keep only good-orientation cases
        df = df[df['good_ori'] == 'Y'].reset_index(drop = True)
    
    train_set = df[(df['DS_split'] == 'train') & (df['USorUK'] == 'US')]['subject_number'].unique().tolist() # train on US subjects only
