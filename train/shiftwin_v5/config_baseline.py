...
class Config():
    ...
    def __init__(self,
        ...
        #########################################################################
        #                            TRAINING DETAIL
        self.EXPORT_PATH = f"/home/efs/ziping/workspaces/dfu/clf_algo_release_202404_rollback/results/shiftwin_v5/" + self.MODEL.strip().lower()
...
if __name__ == "__main__":
    ...
    gs_df = pd.read_csv("/home/efs/ziping/workspaces/dfu/clf_algo_release_202404_rollback/src/data/grid_search_baseline_240502.csv")
    ...
    train_csv = f"/home/efs/ziping/workspaces/dfu/clf_algo_release_202404_rollback/src/data/fixed20240502_processed_WAUSI_BSV+shiftwin_20240304_updated.csv"
    df = pd.read_csv(train_csv)
    
    if image_preprocess['USE_GOOD_ORI_ONLY']: # Keep only good-orientation cases
        df = df[df['good_ori'] == 'Y'].reset_index(drop = True)
    
    train_set = df[(df['DS_split'] == 'train')]['subject_number'].unique().tolist() # train on US subjects only
    test_set = df[df['DS_split'] == 'test']['subject_number'].unique().tolist() # test on US + EU
    
    train_msi = f"/home/efs/Xinyi/DFU/2024Mar_DL_WASP_pretrain/code_old_dataset_baseline_repeat/data/BSV+slidingwindow_Processed_V4_centercrop_20240422/cropped_msi_ulcer_only/morph_{image_preprocess['MASK_DILATION']}"
